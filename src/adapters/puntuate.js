...
#!/usr/bin/env python3
"""FIP World Cup qualifier scraping: group tables + the day's order of play.

TWO SOURCES, because FIP runs its championships on different rails to the tour:

  * GROUPS/STANDINGS - widget.matchscorerlive.com/screen/groups/<msid> (the tour's
    widget, so the same host our other scrapers use). Carries the group tables,
    points, rubbers won, and each tie inside a collapsed block.
  * ORDER OF PLAY / LIVE - postafip.puntuate.com/ordenJuegoFip.aspx?idTorneo=<id>.
    Serves TODAY only (no date parameter); `enjuego=1` is the live view and
    `rf=<seconds>` is an auto-refresh interval, not a tab.

Stdlib only, so publish.ps1's bare system Python can run it.
"""
...
GROUPS_URL = "https://widget.matchscorerlive.com/screen/groups/{msid}?t=tol"
OOP_URL = "https://postafip.puntuate.com/ordenJuegoFip.aspx?idTorneo={tid}&rf=0"
LIVE_URL = "https://postafip.puntuate.com/ordenJuegoFip.aspx?idTorneo={tid}&rf=0&enjuego=1"
...
def get(url: str, timeout: int = 25) -> str:
    req = urllib.request.Request(url, headers={
        "User-Agent": UA, "Referer": "https://www.padelfip.com/"})
...
def parse_groups(page: str) -> list[dict]:
    """[{draw: 'men'|'women', group: 'G', teams: [{pos, code, name, pts, rubbers}],
        ties: [{a, b, score}]}]. Draw order follows the widget: draw-1, draw-2."""
    page = re.sub(r"(?s)<script.*?</script>", " ", page)
    spans = []
    for i, m in enumerate(re.finditer(r'id="draw-(\d+)"', page)):
        start = m.start()
        nxt = re.search(r'id="draw-\d+"', page[m.end():])
        spans.append((("men", "women")[i] if i < 2 else f"draw{i+1}",
                      page[start:m.end() + nxt.start() if nxt else len(page)]))
    out = []
    for draw, seg in spans:
        blocks = re.split(r'<span[^>]*>\s*Group\s+([A-Z])\s*</span>', seg)
        # blocks = [pre, 'A', body, 'B', body, ...]
        for i in range(1, len(blocks) - 1, 2):
            letter, body = blocks[i], blocks[i + 1]
            teams = []
            for row in re.finditer(
                    r'group-index[^>]*>\s*(\d+)\.\s*</div>.*?'
                    r'flags/([A-Z]{3})\.jpg.*?<span[^>]*>\s*([^<]+?)\s*</span>.*?'
                    r'group-pts[^>]*>\s*([^<]*?)\s*</div>.*?'
                    r'group-rubs[^>]*>\s*([^<]*?)\s*</div>', body, re.S):
                pos, code, name, pts, rubs = row.groups()
                teams.append({"pos": int(pos), "code": code, "name": html.unescape(name),
                              "pts": pts.strip(), "rubbers": rubs.strip()})
            # Each team carries its own collapsed list of ties, so the same tie
            # appears twice (once per side). Key on the unordered pair.
            ties, seen_pairs = [], set()
            starts = [m.start() for m in re.finditer(r'<div class="collapse mb-4"', body)]
            for k, st in enumerate(starts):
                block = body[st:starts[k + 1] if k + 1 < len(starts) else len(body)]
                lines = _text(block)
                for j, l in enumerate(lines):
                    if l != "vs" or not j or j + 1 >= len(lines):
                        continue
                    a, b = lines[j - 1], lines[j + 1]
                    score = (lines[j + 2] if j + 2 < len(lines)
                             and re.match(r'^\d+\s*-\s*\d+$', lines[j + 2]) else "")
                    key = tuple(sorted((a, b)))
                    if key in seen_pairs:
                        if score:                      # keep whichever side carries a result
                            for t in ties:
                                if tuple(sorted((t["a"], t["b"]))) == key:
                                    t["score"] = t["score"] or score
                        continue
                    seen_pairs.add(key)
                    ties.append({"a": a, "b": b, "score": score})
            if teams:
                out.append({"draw": draw, "group": letter, "teams": teams, "ties": ties})
    return out


def parse_oop(page: str) -> dict:
    """{'day': 'Tuesday, 22 September 2026', 'ties': [...]} from the Puntuate sheet.
    A tie line reads: 'Match 1 Female - Group Tie 1 - F_F - BGR 0 - 0 GRC'."""
    lines = _text(re.sub(r"(?s)<script.*?</script>", " ", page))
    day = next((l for l in lines if re.match(r'^[A-Z][a-z]+day, \d', l)), "")
    empty = any("No matches" in l for l in lines)
    ties: list[dict] = []
    court_rows: dict = {}
    court = ""
    for i, l in enumerate(lines):
        cm = re.match(r'^COURT\s+(\S+)', l, re.I)
        if cm:
            court = cm.group(1)
        # The group token (F_F / M_G) appeared on 21 Sep's preview sheet and was
        # GONE once play started on 22 Sep - "Group Tie 1 DNK 0 - 0 SRB". Both
        # shapes parse; the group is optional and only used for labelling.
        # Separators drift between sheets and between draws on the SAME day:
        # "Group Tie 1 DNK 0 - 0 SRB" (men), "Group Tie 1 - DEU 2 - 0 HUN"
        # (women, bare dash), "Group Tie 1 - F_F - BGR 0 - 0 GRC" (preview).
        m = re.match(r'^Match (\d+) (Male|Female) - Group Tie (\d+)'
                     r'(?:\s*-\s*([A-Z]_[A-Z]))?\s*-?\s*([A-Z]{3})\s+(\d+)\s*-\s*(\d+)\s+([A-Z]{3})', l)
        if not m:
            continue
        no, gender, tie_no, grp, a, sa, sb, b = m.groups()
        grp = grp or ""
        # Player names follow the row: <NATION A>, two players, <NATION B>, two.
        names = [x for x in lines[i + 1:i + 7]]
        players = [x for x in names if re.match(r'^[A-Z]\.\s', x)][:4]
        when = next((lines[k] for k in range(max(0, i - 3), i)
                     if re.match(r'^(\d{1,2}:\d{2}|Followed by|Not before)', lines[k])), "")
        key = (gender, grp, a, b)
        # Position of this row within its court, so a tie that is second on court
        # is not advertised with the court's own start time. FIP's sheet repeats
        # the session clock ("09:00") on EVERY tie under a court header even
        # though the two ties on a court are interleaved match by match - on
        # 22 Sep it showed the Denmark men's tie as 09:00 when it followed the
        # women's and began around noon. Order is the honest signal, not a clock.
        court_rows.setdefault(court, [])
        first_on_court = len(court_rows[court]) == 0
        court_rows[court].append(key)
        row = next((t for t in ties if t["key"] == list(key)), None)
        if row is None:
            row = {"key": list(key), "gender": "Men" if gender == "Male" else "Women",
                   "group": grp, "a": a, "b": b, "score": f"{sa}-{sb}",
                   "court": court,
                   # Only the first tie on a court keeps the clock.
                   "time": when if first_on_court else "follows the earlier tie",
                   "first_on_court": first_on_court, "matches": []}
            ties.append(row)
        row["score"] = f"{sa}-{sb}"
        row["matches"].append({"no": int(no), "time": when, "court": court,
                               "players": players})
    return {"day": day, "empty": empty, "ties": ties}
...
def main() -> int:
    ...
    what = sys.argv[1]
    if what == "groups":
        data = parse_groups(get(GROUPS_URL.format(msid=sys.argv[2])))
        for g in data:
            nord = [t["name"] for t in g["teams"] if t["name"] in NORDIC]
            print(f"{g['draw']:5} Group {g['group']}: "
                  + ", ".join(f"{t['name']} ({t['pts']}, {t['rubbers']})" for t in g["teams"])
                  + (f"   <- {'/'.join(nord)}" if nord else ""))
            for t in g["ties"]:
                print(f"        {t['a']} v {t['b']} {t['score']}")
    elif what in ("oop", "live"):
        url = (OOP_URL if what == "oop" else LIVE_URL).format(tid=sys.argv[2])
        d = parse_oop(get(url))
        print(f"{d['day']}  ({'nothing scheduled' if d['empty'] else str(len(d['ties'])) + ' tie(s)'})")
        for t in d["ties"]:
            print(f"  court {t['court']:3} {t['time']:14} {t['gender']:5} {t['group']}  "
                  f"{t['a']} {t['score']} {t['b']}   ({len(t['matches'])} match rows)")
    else:
        print(json.dumps(parse_groups(get(GROUPS_URL.format(msid=what))), indent=1))
    return 0
